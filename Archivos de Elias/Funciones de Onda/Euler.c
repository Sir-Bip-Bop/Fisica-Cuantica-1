...
double V0=300.;// Altura del escalon en eV (o cte de energía potencial)

double V(double x) // Funcion que da la energia potencial en eV
{ double x0=280., x1=320., x2=320.1;
if(x<x1) return 0; //Escalon de potencial
//if(x<x1 && x > x0) return 0;  //Pozo cuadrado
//if(x > x1||x < x0) return 0;  //Barrera
else if(x>x2) return V0;
else return V0 *(x-x1)/(x2-x1); // Escalon no discontinuo: V pasa suavemente de 0 a V0 entre x1 y x2

//return 1.e-2*V0*(x-x0)/a*(x-x0)/a;  //Armónico simple centrado en x0
}
main()
{
...
printf("\n Altura escalón: V0 %lf eV, energía: <E> = %lf eV\n k0 = %lf Angs-1",V0,Energ,k0);
...
Norma=sqrt(Norma*dx);
//Normaliza la función dada y guarda el cuadrado en fichero
 ...
 { 
    ...
    do{
     ...
     FI[j]=(CC-AA*BB)/(1+AA*AA);
      Pold=P[j];
      P[j]=FR[j]*FR[j]+FI[j]*FI[j];
      if(fabs(P[j]-Pold)>err) err=fabs(P[j]-Pold);
      Norma=Norma+ P[j];
    }
    } while(err>1.e-6);
    Norma=sqrt(Norma*dx); 
   ...
   Deltax =sqrt(x2med-xmed*xmed); 
    kmed=0.;
    for(j=Nx1+1;j<Nx2;j++)kmed =kmed+(FI[j+1]-FI[j-1])*FR[j]; // k [A-1] = p/hbar medio= 2*Int(FI'*FR*dx)
   if(n%100==0)
   {
